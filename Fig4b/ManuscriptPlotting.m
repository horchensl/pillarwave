% Matlab script to generate Manuscript figure 4b using RAW data
%
% Files needed:
% - 'TXwaveform4b.mat'     :  Transmit wavefrom function [-]
% - 'TxData4b.mat'         :  Measured transmit waves at transducer terminals,
%                             compensated for 1:10 high impedance probe
%                             [Volt], including time traces
% - 'HydrophoneData4b.mat' :  Measured hydophone response [Volt], including time traces
%
% 7 March 2024
...
Hydrophone_sens = 5.3111e-08;       % Hydrophone sensitivity @ 8.2 MHz [V/Pa]
Hydrophone_Distance = 0.0288;       % Hydophone distance from transdcuer surface, derived from time of flight.
Freq_sel = 8.2e6;                   % Selected frequency [Hz]
...
X_loc_offset = 0;                   % Centre alignment offset [mm], X-axis
Y_loc_offset = -2;                  % Centre alignment offset [mm], Y-axis
...
range0 = 1953:2976;     % Range of sample points containing pressure wave (i.e. time windowing time trace)  

%% Load Raw data
load('TXwaveform4b.mat');                 % Lineair frequency sweep transmit waveform 
load('TxData4b.mat');                     % Transmit signal on transducer connector, compensated for 1:10 probe attenuation factor [Volt] and time traces [sec]
us_src_sweep = mean(d_allsweep_src,2);  % Averaging transmit signal across all scan positions, assuming stable excitation

load('HydrophoneData4b.mat');             % Hydophone output [Volt] and time traces [sec]
